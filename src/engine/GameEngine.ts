...
export class GameEngine {
  ...
  submitVote(voterId: string, votedPlayerId: string): void {
    ...
    if (allVoted) {
      round.eliminatedPlayerId = this.tallyVotes(round.votes, round.turnOrder);
      round.phase = RoundPhase.REVEAL;
    }
  }

  /**
   * En çok oyu alan oyuncuyu döner. Eşitlik durumunda `turnOrder` içinde
   * daha ÖNCE gelen oyuncu tercih edilir.
   *
   * DÜZELTME NOTU: Önceki implementasyon `Object.values(votes)` üzerinde
   * dolaşıp Map ekleme (insertion) sırasına göre eşitlik bozuyordu —
   * bu sıra aslında "kim oy verdi" sırasıydı, dokümante edilen niyet olan
   * "turnOrder'daki ilk sıradaki aday" değildi. Artık adaylar (votedId'ler)
   * açıkça turnOrder pozisyonuna göre sıralanıp en yüksek oy sayısına
   * sahip, turnOrder'da en öndeki aday seçiliyor.
   */
  private tallyVotes(votes: Record<string, string>, turnOrder: string[]): string {
    const counts = new Map<string, number>();
    for (const votedId of Object.values(votes)) {
      counts.set(votedId, (counts.get(votedId) ?? 0) + 1);
    }

    const turnOrderIndex = new Map(turnOrder.map((id, idx) => [id, idx]));
    let winner = "";
    let maxVotes = -1;
    let winnerIndex = Number.MAX_SAFE_INTEGER;

    for (const [playerId, count] of counts) {
      const idx = turnOrderIndex.get(playerId) ?? Number.MAX_SAFE_INTEGER;
      const isBetter =
        count > maxVotes || (count === maxVotes && idx < winnerIndex);
      if (isBetter) {
        maxVotes = count;
        winner = playerId;
        winnerIndex = idx;
